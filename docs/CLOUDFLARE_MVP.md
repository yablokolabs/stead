...
| Speaks | Edge `en-GB-SoniaNeural` | the browser, `lang: en-GB` | Sarvam `bulbul:v3`, `ratan` |
...
Speech carries the same `en-GB-SoniaNeural` the Hermes preview uses, which
would finally make both halves of Stead sound identical. Azure's free tier is **0.5
