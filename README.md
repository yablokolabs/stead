...
`en-GB-SoniaNeural` (British female). Sarvam has no British voice, so the two
halves use different providers — see `ARCHITECTURE.md` for why, and for what to
change if Indian English is acceptable. Other British Edge voices include
`en-GB-LibbyNeural` (female) and `en-GB-RyanNeural` / `en-GB-ThomasNeural`
(male).
