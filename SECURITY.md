...
## The web MVP is a different system

Everything above describes the Telegram/Hermes preview: one VM, one profile,
state in a mode-600 SQLite file, four documented egress edges. The web MVP
shares none of it. It runs entirely on hosted services — Cloudflare, Supabase,
n8n Cloud — and **touches no VM at all**. Its boundaries have to be stated
separately because almost none of the reasoning above transfers.

### Identity is the boundary

The browser signs in to Supabase and gets an access token. It receives nothing
else: no n8n webhook URL, no webhook secret, no service-role key. The Cloudflare
Worker verifies that token cryptographically against the project's published
JWKS — asymmetric, ES256, no shared secret to leak — and reads `sub` and `email`
from the verified payload.

The Worker then reads exactly one field from the request body: `message`, or the
recording. Any `user`, `user_id` or `household_id` the browser sends is
discarded rather than sanitised. Identity comes from the verified token or it
does not exist. A test sends attacker-controlled identity alongside a valid
token and asserts that what reaches n8n is the token's subject.

This is the same rule `SteadStore` enforces in SQL by binding `household_id` at
construction, moved one layer out. And it reaches its destination: the agent's
conversation memory is keyed on the verified Supabase user id, not on anything
the browser can influence. Pointing that key at a client-supplied value would
let one household read another.

n8n is reached only through the Worker. The webhook is protected by Header Auth,
checked before the workflow starts, so knowing the URL is not enough.

### What leaves, and to whom

| Edge | Sees |
|---|---|
| Supabase | email address, authentication events |
| Sarvam | the household's recorded speech |
| Google Gemini | the transcript or typed message, and the conversation memory |
| n8n Cloud | everything, since the agent runs there |

Four vendors where the Python preview has four *edges* but keeps the
conversation on one machine. This is the real cost of a hosted architecture and
it is not recoverable by configuration.

Not on that list: the service-role key, which no part of this system uses;
household SQLite, which the web path never touches; and any search vendor, per
the next section.

### There is no web search, and adding one has a price

The Python preview confines queries to a SearXNG on loopback precisely so that
no vendor can associate a household's questions with an account. **The web MVP
cannot do this.** There is no VM in its architecture, SearXNG on `127.0.0.1` is
unreachable from n8n Cloud, and public SearXNG instances disable the JSON API
n8n's `toolSearXng` node requires.

So the agent has no web search. It says so when asked, and the prompt forbids
stating any current fact it could only know by looking up — a price, the
weather, today's headlines.

Enabling search means choosing a vendor that will see household queries.
Firecrawl's `/v2/search` is the obvious candidate because a `FIRECRAWL_API_KEY`
already exists in this project — but note what changes. Firecrawl was adopted
here as a *page reader*, and the **Web search** section above says why:

> queries stay on the local proxy and only pages Stead actually opens reach a
> vendor
...
No calendar, no email, no ability to send, book, pay or change anything in any
other system. The agent has conversation memory and the current date, and
nothing else. Its prompt forbids describing an inbox or a diary it cannot read,
and forbids claiming an action was taken.

Unlike the Telegram preview, none of this is enforced in SQL — there is no
approval state machine on the web path, because there is nothing yet for it to
gate. That asymmetry matters if the web MVP ever gains a tool that acts.
