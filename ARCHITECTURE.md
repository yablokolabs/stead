  ...
  └─ Telegram — private "Stead Preview" bot, allow-listed to her ID
       └─ hermes-gateway-stead-kerstin-demo.service   (systemd --user)
            └─ Hermes profile  stead-kerstin-demo
                 ...
                 ├─ TTS: Hermes built-in `edge`, en-GB-SoniaNeural      ──→ out
...
where `en-GB-SoniaNeural` is a genuine British female voice.
...
`en-GB-Sonia`). Sarvam's own TTS is unaffected by that concern and stays a
one-line fallback.
